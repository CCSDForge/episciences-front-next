...
describe('GET /feed/rss', () => {
  ...
  it('returns 400 for an invalid journal id', async () => {
    ...
    const res = await GET(new NextRequest('http://localhost/feed/rss'), makeContext('../evil'));
  ...
  it('passes through the upstream status when the response is not ok', async () => {
    ...
    const { GET } = await import('../route');
    const res = await GET(new NextRequest('http://localhost/feed/rss'), makeContext('epijinfo'));
  ...
  it('returns 200 with the upstream content type and body on success', async () => {
    ...
    expect(await res.text()).toBe('<rss>content</rss>');
  });
