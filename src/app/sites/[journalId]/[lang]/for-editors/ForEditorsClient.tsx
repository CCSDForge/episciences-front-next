...
export default function ForEditorsClient({
  initialPage,
  lang,
  breadcrumbLabels,
}: ForEditorsClientProps): React.JSX.Element {
  ...
  const { data: pageData, isUpdating } = useClientSideFetch({
    fetchFn: async () => {
      ...
      const rawData = await fetchForEditorsPage(rvcode);
  ...
  const title = titleResult.value || t('pages.forEditors.title');
  ...
  return (
    <MarkdownPageWithSidebar
      ...
      breadcrumbLabels={{
        ...
        current: breadcrumbLabels?.current || t('pages.forEditors.title'),
      }}
      lang={lang}
