...
export default async function VolumeDetailsPage(props: {
  ...
  let volumeData: Awaited<ReturnType<typeof fetchVolume>>;
  let translations: Awaited<ReturnType<typeof getServerTranslations>>;
  let activeJournal: Awaited<ReturnType<typeof getCachedJournal>>;
  ...
  try {
    ...
    [volumeData, translations, activeJournal] = await Promise.all([
  ...
  if (volumeData.rvid !== undefined && volumeData.rvid !== activeJournal?.id) {
  ...
  // Each fetch already resolves to null on failure, so this block cannot reject.
