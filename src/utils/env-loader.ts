...
const log = serviceLogger.child({ service: 'env-loader' });
...
export function getJournalsList(): string[] {
  ...
  try {
    ...
    log.warn({ error }, 'Could not read journals.txt');
...
export function loadJournalConfig(journalCode: string): JournalConfig {
  ...
  if (!isValidJournalId(journalCode)) {
    log.warn({ journalCode }, 'Invalid journal code format');
  ...
  try {
    ...
    if (!envPath.startsWith(baseDir + path.sep)) {
      log.warn({ journalCode }, 'Path traversal attempt blocked');
      const emptyConfig = { code: journalCode, env: {} };
      configCache.set(journalCode, emptyConfig);
      return emptyConfig;
    }
    ...
    log.error({ error, journalCode }, 'Error loading config');
