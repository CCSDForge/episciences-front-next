...
function HomeClientInner({ homeData, language, journalId }: HomeClientProps): React.JSX.Element {
  ...
  const {
    ...
    const validAcceptedArticles =
      acceptedArticles?.data?.filter(article => !!article && !!article.id) || [];
    ...
    const shouldRenderStats =
      statsConfig?.render && stats && Array.isArray(stats) && stats.length > 0;
    ...
    const shouldRenderAcceptedArticles =
      acceptedArticlesConfig?.render && validAcceptedArticles.length > 0;
  ...
  return (
    <main className="home">
      ...
      {shouldRenderStats && <StatisticsSection t={t} i18n={i18n} stats={stats || []} />}
      ...
      {shouldRenderIssues && issues && (
        <>
          <div className="home-subtitle">
            ...
            <Link
              href={`${PATHS.volumes}?type=${VOLUME_TYPE.SPECIAL_ISSUE}`}
              prefetch={false}
              lang={language}
            >
              <div className="home-subtitle-all">
                <div className="home-subtitle-all-text">
                  {t('pages.home.blocks.specialIssues.see')}
                </div>
                <CaretRightGreyIcon size={16} ariaLabel="See more" />
              </div>
            </Link>
        ...
        <>
          <div className="home-subtitle">
            ...
            <Link href={PATHS.articlesAccepted} prefetch={false} lang={language}>
              <div className="home-subtitle-all">
                <div className="home-subtitle-all-text">
                  {t('pages.home.blocks.articlesAccepted.see')}
                </div>
                <CaretRightGreyIcon size={16} ariaLabel="See more" />
              </div>
            </Link>
