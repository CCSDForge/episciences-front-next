...
function NewsTileCard({
  ...
  state,
  onToggle,
}: INewsTileCardProps): React.JSX.Element {
  ...
  return (
    <div
      id={cardId}
      className={
        state === 'blurred' ? 'newsCard newsCard-tile newsCard-tile-blur' : 'newsCard newsCard-tile'
      }
      role="button"
      tabIndex={0}
      onClick={onToggle}
...
export default React.memo(NewsTileCard);
