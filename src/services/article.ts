...
const log = serviceLogger.child({ service: 'article' });
...
export async function fetchArticle(
  ...
  try {
    ...
    log.error({ error, paperid }, 'Failed to fetch article');
    return null;
  }
}
...
export async function fetchExportLink(
  ...
  try {
    ...
    log.error({ error, paperid }, 'Error fetching export link');
    return null;
  }
}
...
export function transformArticleForDisplay(rawArticle: any): FetchedArticle | undefined {
  ...
  if (rawArticle && typeof rawArticle === 'object' && rawArticle['@id']) {
    try {
      ...
      log.error({ error }, 'Error formatting article');
...
export async function fetchArticleMetadata({
  ...
  try {
    ...
    if (!response.ok) {
      ...
      log.warn({ status: response.status }, 'Failed to fetch article metadata');
      return null;
    }

    return await response.text();
  } catch (error) {
    log.error({ error }, 'Error fetching article metadata');
    return null;
  }
}
