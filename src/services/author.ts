...
const log = serviceLogger.child({ service: 'author' });
...
export async function fetchAuthors({
  ...
  try {
    ...
    const response = await apiCall<PaginatedResponseWithAuthorsRange<IFacetAuthor>>(endpoint, {
      apiRoot,
      headers: {
    ...
    log.error({ error }, 'Error fetching authors');
    return {
      data: [],
      totalItems: 0,
      rvcode,
...
export async function fetchAuthorArticles({
  ...
  try {
    ...
    const response = await apiCall<PaginatedResponse<RawAuthorArticle>>(endpoint, {
      apiRoot,
      headers: {
        Accept: 'application/ld+json',
      },
    ...
    log.error({ error }, 'Error fetching author articles');
    return {
      data: [],
      totalItems: 0,
    };
